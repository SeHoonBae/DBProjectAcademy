...
SELECT teq.evalq_seq , teq.question as questionnaire, t.type as questionType
...
-- 수정하기
UPDATE tblTeacherEval_Q SET question = '입력한 질문', type_seq = 1 where evalq_seq = 8;
...
select oc.opencourse_seq, c.name as courseName, t.name as teacherName, 
    (SELECT COUNT(*)
        FROM tblTeacherEval_A tea
            INNER JOIN tblRegiCourse rc
                ON tea.regiCourse_seq = rc.regiCourse_seq
                    where rc.openCourse_seq = 1) as answerVariables
                        from tblteacher t
...
SELECT LISTAGG(content, '\t') WITHIN GROUP (ORDER BY content) as tAvlSubject
    FROM tblExample;

-- c. 평가질문 
SELECT teqfirst.evalq_seq, teqfirst.question as questionnaire , (SELECT round((SELECT COUNT(*)
    FROM tblTeacherEval_A tea
        INNER JOIN tblTeacherEval_Q teq
            ON tea.evalQ_seq = teq.evalQ_seq
                INNER JOIN tblRegiCourse rc
                    ON tea.regiCourse_seq = rc.regiCourse_seq
                        WHERE tea.objective_A = 1 and rc.openCourse_seq = 1 and teq.evalq_seq = 3) /(SELECT COUNT(*)
        FROM tblTeacherEval_A tea
        INNER JOIN tblTeacherEval_Q teq
            ON tea.evalQ_seq = teq.evalQ_seq
            INNER JOIN tblRegiCourse rc
                ON tea.regiCourse_seq = rc.regiCourse_seq
                    where rc.openCourse_seq = 1 and teq.type_seq = 2 and teq.evalq_seq = 3),3)*100 || '%' 
    FROM dual) as ratio1 ,(SELECT round((SELECT COUNT(*)
    FROM tblTeacherEval_A tea
        INNER JOIN tblTeacherEval_Q teq
            ON tea.evalQ_seq = teq.evalQ_seq
                INNER JOIN tblRegiCourse rc
                    ON tea.regiCourse_seq = rc.regiCourse_seq
                        WHERE tea.objective_A = 2 and rc.openCourse_seq = 1 and teq.evalq_seq = 3) /(SELECT COUNT(*)
        FROM tblTeacherEval_A tea
        INNER JOIN tblTeacherEval_Q teq
            ON tea.evalQ_seq = teq.evalQ_seq
            INNER JOIN tblRegiCourse rc
                ON tea.regiCourse_seq = rc.regiCourse_seq
                    where rc.openCourse_seq = 1 and teq.type_seq = 2 and teq.evalq_seq = 3),3)*100 || '%' 
    FROM dual) as ratio2, (SELECT round((SELECT COUNT(*)
    FROM tblTeacherEval_A tea
        INNER JOIN tblTeacherEval_Q teq
            ON tea.evalQ_seq = teq.evalQ_seq
                INNER JOIN tblRegiCourse rc
                    ON tea.regiCourse_seq = rc.regiCourse_seq
                        WHERE tea.objective_A = 3 and rc.openCourse_seq = 1 and teq.evalq_seq = 3) /(SELECT COUNT(*)
        FROM tblTeacherEval_A tea
        INNER JOIN tblTeacherEval_Q teq
            ON tea.evalQ_seq = teq.evalQ_seq
            INNER JOIN tblRegiCourse rc
                ON tea.regiCourse_seq = rc.regiCourse_seq
                    where rc.openCourse_seq = 1 and teq.type_seq = 2 and teq.evalq_seq = 3),3)*100 || '%' 
    FROM dual) as ratio3, (SELECT round((SELECT COUNT(*)
    FROM tblTeacherEval_A tea
        INNER JOIN tblTeacherEval_Q teq
            ON tea.evalQ_seq = teq.evalQ_seq
                INNER JOIN tblRegiCourse rc
                    ON tea.regiCourse_seq = rc.regiCourse_seq
                        WHERE tea.objective_A = 4 and rc.openCourse_seq = 1 and teq.evalq_seq = 3) /(SELECT COUNT(*)
        FROM tblTeacherEval_A tea
        INNER JOIN tblTeacherEval_Q teq
            ON tea.evalQ_seq = teq.evalQ_seq
            INNER JOIN tblRegiCourse rc
                ON tea.regiCourse_seq = rc.regiCourse_seq
                    where rc.openCourse_seq = 1 and teq.type_seq = 2 and teq.evalq_seq = 3),3)*100 || '%' 
    FROM dual) as ratio4, (SELECT round((SELECT COUNT(*)
    FROM tblTeacherEval_A tea
        INNER JOIN tblTeacherEval_Q teq
            ON tea.evalQ_seq = teq.evalQ_seq
                INNER JOIN tblRegiCourse rc
                    ON tea.regiCourse_seq = rc.regiCourse_seq
                        WHERE tea.objective_A = 5 and rc.openCourse_seq = 1 and teq.evalq_seq = 3) /(SELECT COUNT(*)
        FROM tblTeacherEval_A tea
        INNER JOIN tblTeacherEval_Q teq
            ON tea.evalQ_seq = teq.evalQ_seq
            INNER JOIN tblRegiCourse rc
                ON tea.regiCourse_seq = rc.regiCourse_seq
                    where rc.openCourse_seq = 1 and teq.type_seq = 2 and teq.evalq_seq = 3),3)*100 || '%' 
    FROM dual) as ratio5
    FROM tblTeacherEval_Q teqfirst
        WHERE teqfirst.type_seq = 2;
    ...
    FROM tblTeacherEval_A tea
        INNER JOIN tblTeacherEval_Q teq
            ON tea.evalQ_seq = teq.evalQ_seq
                INNER JOIN tblRegiCourse rc
                    ON tea.regiCourse_seq = rc.regiCourse_seq
                        WHERE tea.objective_A = 5 and rc.openCourse_seq = 1) /(SELECT COUNT(*)
        FROM tblTeacherEval_A tea
            INNER JOIN tblRegiCourse rc
                ON tea.regiCourse_seq = rc.regiCourse_seq
                    where rc.openCourse_seq = 1),3)*100 || '%' as ratio
    FROM dual;
