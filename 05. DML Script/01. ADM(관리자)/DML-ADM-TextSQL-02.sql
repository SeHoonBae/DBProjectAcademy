...
select subject_seq, name, period from tblSubject;

-- 등록하기(강의 예정인것에 한함)
-- 조건(시간)
SELECT 
    case
        when startDate > sysdate then 'true'
        else 'false'
    end as timeIf
        FROM tblOpenCourse; -- 과정 시작일
...
INSERT INTO tblOpenSubjectMgmt 
    VALUES(OpenSubjectMgmt_seq.nextval,to_date('입력한 시작일', 'yyyy-mm-dd')
...
-- 개설과정 종료일 수정
update tblOpenCourse Set endDate = to_date('','yyyy-mm-dd') where opencourse_seq = ;
