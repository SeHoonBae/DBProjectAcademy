...
SELECT cr.consult_seq , stu.name as studentName, t.name as teacherName, c.name as courseName, stu.tel as studentTel, stu.major as studentMajor, cr.requestDate as callRequestDate, cr.requestContent as callRequestDetails
...
-- b. 출력구문
SELECT oc.course_seq as seq, c.name as courseName, oc.startDate || '~' || oc.endDate as coureseDuration, t.name as teacherName, r.roomName as className
    FROM tblCourse c
        INNER JOIN tblOpenCourse oc
...
select oc.opencourse_seq, c.name as courseName, oc.startdate || '~' || oc.enddate as courseDuration, t.name as teacherName, r.roomName as className
    from tblTeacher t
        inner join tblTeacherCourse tc
...
select record.record_seq, record.recorddate, t.name as teacherName, stu.name as studentName, request.requestcontent, record.recordcontent
    from tblConsultRecord record
        inner join tblConsultRequest request
            on record.consult_seq = request.consult_seq
                inner join tblRegiCourse rc
                                    ...
                                    on oc.openCourse_seq = rc.openCourse_seq
                                        inner join tblTeacherCourse tc
                                            on oc.openCourse_seq = tc.openCourse_seq
                                                inner join tblTeacher t
                                                    on tc.teacher_seq = t.teacher_seq
                                                        where oc.opencourse_seq = '선택한 번호';
