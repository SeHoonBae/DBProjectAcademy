...
SELECT s.student_seq, s.name as studentName, s.pw as studentPw, c.name as courseName, oc.startDate || '~' || oc.endDate as courseDuration, r.roomName as className
    FROM tblStudent s
...
select regicourse_Seq from tblattendancemgmt;
...
select  s.student_seq, s.name as studentName, to_char(admg.workon, 'hh24:mm') as commuteTime, to_char(admg.workoff, 'hh24:mm') as quittingTime, admg.state as attendanceSituation
    from tblstudent s
        inner join tblregicourse rc
            on s.student_seq = rc.student_seq
                inner join tblopencourse oc
